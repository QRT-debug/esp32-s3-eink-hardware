...
### 七、UART 单线（不回读）的风险评估与验证计划（2026-09-13，用户提问"不回读查不出问题吗"）

**为什么单线够用（三层理由）**：
1. **关键状态有专用硬件脚，不依赖串口**：连接/断开 = LED_A（GPIO40，高有效），这是硬件级状态线，比串口回执更可靠。
2. **厂家指令集本来就极简**：全部约 20 条，应答只有 `+OK`/`+ERROR` 单行；本项目用到的 6 条（VOLP/VOLM/PLAYPAUSE/NEXT/PREV/BTDISCON）全是**效果可观察**的单发指令——发了就生效，听不听得到/连没连上（LED_A）就是回执。没有任何一条"必须回读才能用"的查询型指令（MAC/版本只在产测时看一次）。
3. **指令无破坏性**：最坏情况 = 指令被忽略（+ERROR 我们没看见），不存在"发错导致损坏"的可能，风险上限很低。

**放弃的东西与对价**：

| 放弃 | 影响 | 对策 |
| --- | --- | --- |
| 指令回执（+OK/+ERROR） | 配置类指令（AT+NAME 等）失败无感知 | 只在产测/装机时配置一次，台架验证；运行期指令效果可观察 |
| 查询 MAC/版本 | 运行期用不到 | 台架用 USB-TTL 读一次即可 |
| 异步通知（如来电） | HFP 不用，无影响 | — |
| 音量绝对值回读 | 固件音量计数器与模块内部可能漂移 | 相对调节 + 固件自己记账，够用；真要精确再议 |

**开发期验证计划（不占板载 GPIO）**：台架上用 **USB-TTL 小板监听模块 22 脚 TXD**（模块 TXD → 适配器 RX），一次性验证：① 上电默认是否直接接受 AT（是否需要先发 `+++`）；② 各指令的 `+OK/+ERROR` 返回格式与时序；③ LED_A 翻转与真实连接事件的对应关系。验证完的结论写死进固件，量产不需要回读。

**容错设计（双音源架构的额外好处）**：模块死机/挂起只损失"蓝牙音乐"这条路——`SEL` 默认 = PCM 路径、功放 EN 独立，**ESP32 播报路径不受任何影响**；模块异常的兜底 = 断电重启（LED_A 长时间为低 + 用户侧异常即可判）。
...
- 去耦 **C12 = 0.1 µF 0402**（C1525，画布实证；BOM 已明确 C12=CH340N，原记 0603 为笔误）。
...
5. 高频去耦用小封装（ESL 小），0.1 µF 一律 0402（2026-09-14 统一规则）并贴着引脚。
6. 封装越大越怕板弯开裂，板边与螺丝孔附近避免 1206 以上。

### 本板封装策略（2026-09-11 定稿，2026-09-14 按统一规则修订）

**结论（2026-09-14 用户定稿）：0.1 µF 一律 0402；1 µF / 10 µF / 22 µF 一律 0805。** 原"全板统一 0603/0805、不沿用参考图的 0402"的方案作废——实际画布去耦本就是 C1525（0402），小封装 ESL 更低、高频去耦更优。详见 BOM.md「电容封装统一规则」节。判据保留：

- **储能/滤波类**：EN 的 1 µF、TP4056 与 TPS63021 的输入输出电容、各芯片 0.1 µF——按统一规则放料（0.1 µF→0402、1/10/22 µF→0805）；同容值下封装越大 DC bias 衰减越小，10 µF 选 0805 正是此理。
- **必须保持原封装**：调谐/高频类——射频匹配、晶振负载、PLL 环路、高速旁路（封装越大 ESL 越大、自谐振频率越低）；以及 ESD/TVS 器件（USBLC6-2SC6 的 SOT-23-6 封装即性能，不可换）。
- 本板为模组方案，天线与匹配都在模组内、板上无射频调谐元件，因此**没有任何一个元件封装是"必须保持"的**；0.1 µF 统一 0402 后 ESL 反而更优。
