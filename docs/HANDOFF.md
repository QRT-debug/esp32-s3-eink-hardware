...
- **Type-C 段（2026-09-12 03:48 版本，84 器件/57 网络）：主体正确，2 处待修**：
  ...
  - 备注：VBUS1/VBUS2 目前为自动名 N210/N229（电气正确，可选改名）；wireCrossings=12 为视觉走线交叉、不影响电气；位号已按顺序重排；后续待放顺延：R10=CHRG 上拉（GPIO17）、R11/R12=I2C 上拉。**Type-C 段核对通过 ✓ 阶段 1 剩余：J5 电池连接器（待画）。**
- **电池保护段 + 电量计段（2026-09-12 05:41 版本，102 器件/62 网络）：全对 ✓ 阶段 1 主体完成**：
  ...
  - ⚠️ 建议：C13 一端现接 GND，建议改接 **BAT_N**（与 VSS 同侧，保护触发时 DW01A 去耦更稳；不强制）
